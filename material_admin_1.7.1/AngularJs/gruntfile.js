...
module.exports = function(grunt) {
        ...
        options: {
          ...
          middleware: function(connect, options) {
            ...
            options.base.forEach(function(base) {
                proxy.push(connect.static(base));
            });

            // 让目录可被浏览（即：允许枚举文件）
            var directory = options.directory || options.base[options.base.length - 1];
            proxy.push(connect.directory(directory));
  ...
  grunt.registerTask('serve', ['dom_munger:read', 'configureProxies:main', 'connect:main', 'watch']);
  grunt.registerTask('serve_local', ['dom_munger:read', 'configureProxies:local', 'connect:local', 'watch']);
